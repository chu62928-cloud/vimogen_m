...
本分支实现 ViMoGen 骨盆姿态控制的 M1–M7 统一比较框架。历史 C0/S0/S1 使用根旋转代理角，已因前倾符号和整个人倾倒问题暂停用于解剖骨盆前后倾主张。当前新增的局部前后倾 S1 是**骨盆—胸廓相对角单目标探索实验**，与下文历史 C0 的“S1”不是同一实验。

当前结论不是最终论文结论。旧 C0 的 S0 物理闭环、S1 筛选和冻结结果保留在下文及 [结果账本](result.md)；新版相对角 S1 仅完成样本 94、种子 0 的开发矩阵，尚未完成七方法配对验证、足底接触评价和用户视频终验。

面向论文写作的累计结果、结论边界和产物索引统一维护在 [结果账本](result.md)；后续每次新增、重跑或冻结实验均同步更新该文件。

## 新版 S1：骨盆局部前后倾探索（2026-09-19）

目的：检验**只把控制目标改为骨盆相对胸廓的前后倾**，能否让骨盆本身向正确方向转动，同时避免旧方案的整体后倒。正剂量定义为骨盆前侧相对胸廓进一步下沉。整体刚体旋转不会改变该相对角；S1 不加入胸廓、足部或轨迹保护损失。M3/M4/M7 为使相对角可优化，开放了多关节局部旋转求解，这不是额外保护约束。

过程：在服务器相同的样本 94、种子 0、配对 M0 和剂量 −5°/0/+5° 上，M1–M7 分别尝试中心强度的 0.5、1、2 倍；每条结果先检查相对角 MAE≤1°、P95≤2°及方向。未命中相对角的结果不进入骨盆世界角、胸廓或接触结论。M4 另将采样端点和末端校正分开审计；M7 只作生成后编辑参考。代码入口为 [开发矩阵](experiments/run_local_pelvis_s1_matrix.py)、[统一评价](evaluation/local_pelvis_metrics.py)、[M4 前后审计](experiments/audit_local_pelvis_m4_terminal.py)和[侧视渲染](experiments/render_local_pelvis_s1_pair.py)。

| 方法 | 开发条件中非零剂量相对角命中 | 解释 |
|---|---:|---|
| M1 | 6/6 | 生成期命中，但骨盆世界角未按剂量前倾 |
| M2 | 待定 | 重复批量与批量—单样本一致性审计尚未完成 |
| M3 | 0/6 | 当前强度与预算内欠响应；按目标门停止后续分析 |
| M4 | 采样端点 0/6；末端校正后 6/6 | 只能把命中归于末端编辑 |
| M5 | 1/6 | 仅双倍强度 +5° 命中，不构成稳定成功 |
| M6 | 4/6 | 中心与双倍强度生成期命中，仍有胸廓代偿 |
| M7 | 6/6 | 输出空间编辑可行，不代表生成期引导成功 |

关键反例：M1 中心配置 +5° 的相对角响应为 +5.116°（MAE 0.431°），骨盆世界角却变化 −0.718°，胸廓世界旋转 P95 为 4.973°；M6 中心 +5° 相应为 +4.471°、−0.522°和 4.390°。因此**相对角命中不等于骨盆实际前倾**。固定 M0 相机的 M1 +5° 网格侧视预览已生成 100 帧、20 帧/秒，但全量视频、接触数值和用户终验尚未完成；当前无完整任务通过结论。下一阶 S2 仅在完成 S1 审计后，针对已证实的“骨盆未动、胸廓代偿”问题增加骨盆世界角目标。

历史 C0/S0/S1 的详细过程和结果保留如下，不能与新版 S1 的通过率合并。

## 历史 C0 状态（冻结）

- 当前工作分支：`codex/pelvis-m1-m7-scale-experiments`
- 冻结协议：`vimogen_pelvis_m1_m7_scale_v1`
- 约束包：历史 C0，仅控制全局根旋转代理角，不能解释为解剖前倾
