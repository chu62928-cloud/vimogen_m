...
- S1 屏选实时快照：总进度已为 `25/40` 组；M1、M3、M4 各完成 `8/8` 组，M5 完成 `1/8` 组。M1 各组角度/数值均为 `4/4`，物理 v2 均为 `0/4`；M3 各组数值均为 `4/4`但角度均为 `0/4`；M4 各组角度/数值均为 `4/4`但物理 v2 均为 `0/4`。尚未生成屏选选择文件。
