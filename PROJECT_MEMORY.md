...
- VERIFIED/DEVELOPMENT：服务器 `local_pelvis_s1` 的 M1、M3、M4、M5、M6 各三档强度×−5/0/+5°，共 45 条开发槽位均有有效评价；配对 M0 哈希在同一样本/种子内一致。只读汇总为 `results/phase9/pelvis_m1_m7/local_pelvis_s1/development_audit_attempt_01.json`，本地副本在同目录的 `local_inspection/`。这些不是 196 槽位配对验证，也不是完整数值通过：接触网格标志尚未物化、用户视频终验未完成。
- VERIFIED/CONTROL：M1 三档、M4 三档、M6 中心/双倍在 ±5° 两条均命中相对角门；M3 三档 0/2；M5 仅双倍 +5° 命中。所有已命中的配置在同一剂量下均未通过骨盆世界角和胸廓保持门。典型 M1 中心 +5°：相对角响应 `+5.116°`、MAE `0.431°`，骨盆世界角 `−0.718°`、胸廓旋转 P95 `4.973°`。这支持 S1 单目标会由胸廓代偿，具备 S2 启动理由，但不能在 M2/M7 未齐时宣称七方法结论。
- VERIFIED/VIDEO_SMOKE：M1 中心、sample94/seed0/+5° 的固定 M0 相机网格侧视冒烟已生成 100 帧、20 fps 的 640×720 干净版及 1280×720 并排版，目录 `local_pelvis_s1/videos/m1_center_p94_s0_d+5/attempt_03/`。前两次因导入及路径错误保留失败记录；第三次有 `summary.json`。并排版尚无解剖标志/胸廓轴/接触叠加，不可称为最终诊断版；动态目视与用户终验待办。
- IN PROGRESS：M7 三档（每档 −5/0/+5°，与当前生成 M0 配对）任务已提交，日志 `local_pelvis_s1/logs/m7_development.log`；M2 真实重复批量与批量—单样本一致性四臂任务已提交，日志 `local_pelvis_s1/logs/m2_invariance_attempt_01.log`。先核对进程和结果再记录为通过或失败；不得把实现阻塞记成方法失败。
- PENDING：M4 末端修正前后分开评价；共享 M0 接触标志/地面评价；S1 所有七方法选参及 196 槽位验证；标志叠加的全部侧视视频和用户终验。S2 的相对角+骨盆世界角实现已开始，但不得混入 S1 候选或覆盖旧 C0；S1 结果须先审计归档。
- NEXT：先检查 M7 `progress.json`、M2 `progress.json`/`invariance_audit.json` 和相应日志；随后补 M4 修正前后、网格接触指标和诊断侧视渲染，再冻结每方法开发配置。

## 2026-09-18：S1 相对角实现完成，开发矩阵正在运行

- IMPLEMENTED：新增 `geometry/local_pelvis.py` 与 `evaluation/local_pelvis_metrics.py`。控制量为胸廓（spine3）坐标系中的骨盆矢状相对角，正号表示骨盆前侧下沉；共享任务已接入 M1–M7。M3/M4/M7 的 S1 版本使用多关节自动微分最小范数更新，不再使用对相对角导数为零的纯根单变量。
- VERIFIED/PREFLIGHT：服务器 `mdm5090` 环境专项回归为 `28 passed`，覆盖整体刚体旋转不变、正负号、纯根梯度近零、脊柱梯度非零、最小范数投影和评价恒等性。`git diff --check` 通过；本地 Python 无 PyTorch，因此数值测试以服务器结果为准。
- VERIFIED/M7：样本94、seed0、剂量−5/0/+5° 的 M7 局部最小范数投影 `3/3` 命中相对角门。冻结候选 M0 文件不含源噪声，运行记录显式标记 `NOT_PRESENT_IN_REFERENCE_M7_UNUSED`；同动作/种子的重复通用/种子专属行按文件后出现的种子专属行确定性选取。M7 结果只证明输出空间编辑可行，不证明生成期引导成功。
- VERIFIED/GENERATION_PROBE：样本94、seed0、+5° 中，M1 中心强度响应 `5.116°`，MAE `0.431°`、P95 `0.804°`；M4 响应 `5.000°`；M6 响应 `4.471°`、MAE `0.530°`，三者均命中相对角门。M3 仅响应 `0.037°`；M5 响应 `6.018°`、MAE `1.018°`，两者未命中。
- VERIFIED/S1_LIMITATION：上述命中方法没有得到所需的骨盆实际转动。M1/M4/M6 的骨盆世界角平均增量分别约 `−0.718°/+0.012°/−0.522°`，胸廓世界旋转 P95 分别约 `4.973°/5.061°/4.390°`。这证明相对角单目标主要通过胸廓反向运动达标，满足启动 S2“骨盆世界角增量目标”的条件；在 S1 三强度与±5/0开发矩阵完成前不提前混入 S2。
- RUNNING：服务器单路后台任务 `run_local_pelvis_s1_matrix.py` 正在执行 M1/M3/M4/M5/M6、样本94/seed0、−5/0/+5°、0.5/1/2倍主参数；按现有评价结果断点复用，不并行争用 GPU。日志为 `results/phase9/pelvis_m1_m7/local_pelvis_s1/logs/development_matrix.log`。M2 仍需新版 S1 批量—单样本一致性审计，未计入完成。
- NEXT：完成 S1 开发矩阵后冻结每方法一套配置，扩展到两动作×两种子×核心四剂量并渲染共享 M0 相机的干净版/诊断版侧视视频；同时实现 S2，保持相同变量集合，只新增骨盆世界角增量目标。用户目视终验前不得标记完整任务通过。

## 2026-09-18：局部骨盆 M1–M7 修正实施方案已制定，尚未运行

- USER_SCOPE：用户同意尝试局部相对角引导，要求结合文献提出并共同实施多条解决路线。本轮完成具体实施设计，没有启动新版模型实验或训练。
- VERIFIED/CODE：当前 M3/M4 采用根单自由度、旧角度导数为 1 的投影；M7 直接修改根旋转。新骨盆—胸廓三维相对角对整体根旋转不变，因此必须扩展到局部关节协调，不能仅替换配置。多种方法目前还明确拒绝非 C0 约束包。
- PLANNED：新增 `LOCAL_PELVIS_PILOT_PLAN_20260918.md` 和 `configs/m1_m7/local_pelvis_pilot_v1.json`，状态均为 `DRAFT_NOT_RUN`。A 为相对角最小消融；B 为相对角、世界骨盆角、胸廓与冻结接触联合保护；C 为根绕髋中点旋转、三条子树逆补偿及足部逆运动学；D 为最佳两种生成方法的末端/生成中接触投影对照。检索依据与适用边界见方案，不将文献效果当本项目阳性结果。
- BUDGET/PLANNED：P1 输出空间 28 槽位；P2 单例最多 45；P3 两动作/两种子/七剂量最多 420；M2 一致性仍阻塞时 P3 为 364 可执行、56 阻塞。P4 最多新增 112。均为计划上限而非已启动任务；零剂量复用 M0，只有全部来源哈希相同的冒烟输出允许复用。
- INVARIANTS：保留历史 C0、旧结果与独立确认集；新协议门槛在 P3 前冻结。相对角单目标可被胸廓反转满足，完整验收必须包括世界骨盆变化；回退 M0 计控制失败，无有效接触不计通过。适配器训练和物理仿真仅作有条件后续路线。
- NEXT：先实现 `geometry/local_pelvis_v1.py` 的新校准与三维相对角、补充符号/刚体不变性/雅可比/零剂量测试；通过服务器预检后实现 C 并运行 P1，之后才接入 M1–M7 A/B。新运行器及新版算法当前尚未实现，协议 JSON 不可交给旧运行器执行。

## 2026-09-18：骨盆正负号与整体倾倒根因完成复核

### 已验证事实

- VERIFIED/NEGATIVE_RESULT：当前 M1–M7 C0 指标以 SMPL-X 根局部 `+z` 为人体前方，却把该前方向世界上方抬起定义为正；项目要求的解剖骨盆前倾是前侧下沉为正。冻结 M7 全部 `56` 条记录上，校正后的解剖代理增量严格等于当前报告增量取反。当前 `+10°` 不是解剖前倾，而是约 `−10°` 的解剖标志角变化。
- VERIFIED/TENSOR_PROBE：服务器直接读取 sample94、seed0、M7 `±10°` 物理动作张量，并按已审计结论交换历史 v4 中误标的 ASIS/PSIS。当前 `+10°` 的根代理角均值为 `+10.000000°`，校正解剖标志角均值为 `−9.999952°`，躯干有符号前倾变化为 `−10.000000°`（后倒）；当前 `−10°` 对应 `+9.999954°` 解剖标志角和 `+10.000000°` 躯干前倒。
- VERIFIED/RIGID_BODY：M7 全部非零剂量中，世界躯干方向 P95 与绝对剂量的最大差仅 `0.016678°`；21 个局部关节旋转均值最大值为 `0°`，根局部 21 关节 P95 最大值为 `0.000238753 mm`，根平移 P95 为 `0 mm`。sample94 `+10°` 的世界躯干方向平均变化 `9.988198°`、双足世界方向平均变化 `8.601753°`。因此角度命中来自全局根节点带动全身子树，不是骨盆相对躯干/大腿的局部前后倾。
- VERIFIED/CANDIDATE_IMPACT：sample94/seed0/current `+10°` 下，M1 `scan_08`、M4 `scan_05`、M5 `scan_06`、M6 `scan_05/07` 的世界躯干方向 P95 分别约 `7.768°/10.115°/8.423°/7.681°/8.217°`；它们在校正解剖语义下的实际角均为负。M3 倾倒较小但只响应约 `0.633°`，不能作为控制成功。
- REPRO：新增 `experiments/diagnose_pelvis_sign_and_rigid_lean.py`。带 `--fail-on-current-semantics` 的确定性红灯命令连续两次退出码均为 `1`，机器状态为 `CURRENT_CONTROL_SEMANTICS_FAIL`，同时标记 `sign_semantics_reversed=true` 与 `m7_is_rigid_whole_body_rotation=true`。新增 `experiments/probe_existing_m7_tensor_semantics.py` 对远端原始张量完成独立复核。
- ARTIFACTS：本地诊断目录为 `diagnostics/pelvis_sign_rigid_lean_20260918/`，含汇总 `diagnosis.json`、中文 `REPORT.md` 和远端 `±10°` 两份张量探针 JSON。服务器镜像目录为 `/root/autodl-tmp/vimogen_m1_m7_scale/results/phase9/pelvis_m1_m7/pelvis_sign_rigid_lean_20260918/`。四份本地产物 SHA256 依次为：汇总 JSON `7623cf3a…f2a3`、最终报告 `b9724ae8…0347`、`−10°` 张量 JSON `4475cc4e…4d2b`、`+10°` 张量 JSON `51ffe381…4621`。
- FAILED/RESOLVED：第一次远端探针在导入阶段因 `/tmp` 不含项目模块路径停止；第二次因 sample94/seed0 存在两条历史 M0 被唯一性门阻断。最终版本加入项目根路径，并按冻结 `paired_m0_sha256` 唯一选中权威 M0 后完成。两次失败均在指标计算前停止，没有生成、修改或覆盖动作。
- SECURITY：用户提供的 SSH 密码只通过交互式安全输入进入临时进程环境，命令结束后立即清除；未写入代码、项目记忆、日志或产物。

### 决策与后续

- DECISION：立即暂停当前开发前候选冻结、8 动作开发扩样和独立确认。旧 S0/S1、细化和视频结果保留为历史“全局根前侧上抬代理角控制”证据，不得继续表述为解剖骨盆前后倾控制，也不能仅交换视频标签后用于正式结论。
- NEXT_PROTOCOL：另建版本化协议，不覆盖旧 C0。用校正 `A−P` 标志轴定义“前侧下沉为正”，并将主目标改为骨盆相对躯干的局部角变化；世界躯干有符号变化、双足世界方向、根轨迹、穿地/滑动和局部变化占比设为硬门。
- FIRST_ACTION：先实现输出空间解析强基线：根绕双髋中点旋转，`spine1/left_hip/right_hip` 做逆旋转补偿以保持三条子树世界朝向，再用根平移和下肢逆运动学恢复足底接触。只有该原型在 `±5°/±10°` 上通过相对角、躯干和接触门，才接回 M1/M3/M4/M5/M6 的生成引导。

## 2026-09-17：本地无用中间结果清理完成

### 已验证事实

- VERIFIED：清理覆盖本地五个 `vimogen0809*` 工作区，共释放 `724.13 MiB`；当前本工作区占用 `596.45 MiB`。
- CLEANED：删除无外部引用、未被 Git 跟踪且已由正式结果取代的 `diagnostics/candidate_grid_analysis_v2_20260916_attempt_01_incomplete/`（`668.64 MiB`）与 `diagnostics/candidate_grid_analysis_v2_20260916_attempt_02_incomplete/`（`12.37 MiB`）。前者主要由 15 张未压缩 TIFF 构成；正式 `candidate_grid_analysis_v2_3_20260916/`、`candidate_refinement_postprocess_v2_4_20260917_attempt_01/` 和 `candidate_walk_grid_videos_exploratory_v1_20260917_attempt_05/` 均已复核存在。
- CLEANED：删除五个工作区的 Python 与 pytest 等可重建缓存；复核 `__pycache__`、`.pytest_cache`、`.mypy_cache`、`.ruff_cache` 剩余目录数为 `0`。
- PRESERVED：所有正式结果、失败尝试与审计证据、冻结协议、日志、指标表和视频均保留；Git 状态未出现已跟踪文件删除。实验状态与结论未变化，因此 `result.md` 不需更新。

## 2026-09-17：开发前走路候选侧视网格视频完成

### 已验证事实

- COMPLETE_EXPLORATORY_NOT_CONFIRMATION：样本94、seed0的开发前走路可视化已完成。8组配置为M1 `scan_08`、M3 `scan_08`、M4 `scan_05/02`、M5 `scan_06`、M6 `scan_07/05`、M7 `reference`；每段均含共享M0和−10/−5/−2/0/+2/+5/+10°七档剂量。正式确认视频硬门未改变，确认集未读取，开发扩样未启动。
- VERIFIED：最终服务器目录为 `candidate_walk_grid_videos_exploratory_v1_20260917_attempt_05/`；本地交付目录为 `diagnostics/candidate_walk_grid_videos_exploratory_v1_20260917_attempt_05/`。共8段H.264网格MP4、8张中间帧预览、5600行逐帧指标、56行整段指标和机器摘要。每段均为1920×1080、20 fps、100帧、5秒；FFmpeg逐段完整解码通过。
- VERIFIED：配对M0权威哈希为 `ca579a70caef7b2f6ade980cb92b429539cbfcd179dbe2fa6424469616f2ef46`；候选、M0、评价表和渲染输入哈希严格一致。交付压缩包SHA256为 `3d15c5b32137966f53bb2f9e16af8eb98841ceb3345a10573f8198d929883196`。
- VERIFIED：最终服务器相关回归为共享渲染器 `4 passed`、探索网格入口 `5 passed`；所有渲染和FFmpeg进程均正常退出，无残留任务。
- VERIFIED/VISUAL：8张原尺寸预览和联系表已人工检查。M0固定左上，七档剂量顺序一致，人物全身和足部完整入镜，文字、逐帧角度曲线、根局部21关节偏差、根平移、穿地和近地滑动摘要均可读且无裁切、重叠或错格。
- DIAGNOSTIC_ONLY：单一样本中M1 `scan_08`、M4 `scan_05`、M5 `scan_06`、M6 `scan_05/07` 和M7均为7/7角度通过，M3 `scan_08`仅1/7；但M4 `scan_05`与M7的大剂量穿地/滑动增量明显，角度精确不能解释为自然度通过。M6 `scan_05`在该样本的局部姿态偏差和滑动增量略低于 `scan_07`，该观察可辅助人工判断，但不能替代完整开发或确认结果。
- FIXED：首次真实渲染因工作目录不在运行时根目录失败；第二次因使用错误的四条M0参考被哈希硬门拒绝；第三次暴露评价动作是 `[1,T,276]`；第四次进入PyTorch3D后因当前RTX 4080 SUPER计算能力8.9无可用栅格化内核失败。最终统一到项目权威张量哈希、单序列批加载和显式Pyrender（EGL）网格后端，3帧真实冒烟与全量57条源视频均通过。失败日志和空失败目录保留，未覆盖。
- VERIFIED：服务器连接当前使用 `connect.westc.seetacloud.com:47170`、用户 `root`；项目连接脚本默认端口已更新。认证信息只在运行时提供，不写入代码、日志或项目记忆。

### Immediate continuation

1. 由用户审看8段MP4，重点比较M4 `scan_05/02`及M6 `scan_07/05`的步态、足部接触和大剂量自然度；视频只允许影响开发前候选冻结，不能作为确认结果。
2. 审看后明确M6是否同时保留 `scan_05` 和 `scan_07`；M3若保留只能标为欠响应探索性负对照。随后用已验证冻结器生成开发前冻结文件，每方法最多两组。
3. 冻结完成后才启动8动作×3种子开发扩样；开发分析不重新选参，之后创建确认前冻结并运行隔离的4动作×3种子确认。

## 2026-09-17：细化与新版后处理完整结束，候选尚未冻结

### 已验证事实

- COMPLETE：2026-09-17 02:02（服务器 CST），预注册细化阶段达到 `112/112` 个有效完成批次，分配置均为14批：M1/M3/M5/M6 的 `scan_07/08` 各14批。共生成448条方法动作，复用8条配对M0和56条M7参考；新版评价为 `504/504`，失败0。`progress.json` 仍保留1条此前外部资源中断导致的 `GENERATION_FAILED` 记录，同配置成功 `attempt_02` 已纳入112个唯一完成键；该资源记录不计作算法失败。
- COMPLETE_NOT_FROZEN：自动后处理目录为服务器 `candidate_refinement_postprocess_v2_4_20260916_attempt_01/`。旧六点1,736条与细化504条严格合并，来源共2,240条；仅56条重复M7参考被一致性去重，最终为2,184条、39个配置。机制诊断覆盖M1/M3/M4/M5/M6；原始全身MPJPE未参与选择，正式Physical V2未改变，确认集未读取。
- VERIFIED：四动作细化集的自动开发候选建议为：M1 `scan_08`；M3 `scan_08` 仅作控制优先参考且没有满足控制门的综合候选；M4 控制优先 `scan_05`、综合候选 `scan_02`；M5 `scan_06`；M6 `scan_07`。这些状态为 `DEVELOPMENT_CANDIDATES_AFTER_REFINEMENT`，不是已冻结参数，也不构成全局最优声明。
- VERIFIED：M1 `scan_08` 是本轮唯一新增后进入控制可接受集的配置，核心角度通过率 `93.75%`、±10°通过率 `75%`；相较 `scan_07`，低裁剪阈值主要改善大负角度。M3即使把最多投影次数由2提高到8，核心角度通过率仍为0，核心角度MAE仅由约 `3.44°` 降至 `3.05°`，说明不是单纯增加投影预算即可解决。M5新增更高惩罚点控制和内容均未改善，故回到旧 `scan_06`。M6 `scan_07` 核心通过率 `93.75%`，但±10°通过率仅 `31.25%` 且内容偏差高于旧 `scan_05`，冻结前仍需人工决定是否保留两组权衡候选。
- VERIFIED：v2.4 共生成16组图，每组均有PDF/SVG/PNG，共48个图文件；连同合并表、源数据、脚本和诊断共60个文件。已完整下载到本地新目录 `diagnostics/candidate_refinement_postprocess_v2_4_20260917_attempt_01/`，未覆盖任何旧结果。服务器与本地的顶层汇总、候选建议和配置汇总哈希逐字一致，分别为 `55009028…afc0b`、`312a0ee0…a7b3`、`90466cbe…9f4a`。
- VERIFIED/VISUAL：已人工检查M1/M3/M4/M5/M6的参数响应图和七剂量图。坐标、动作颜色、种子点形、绝对近地速度与配对M0增量分面均正确；细化点使用独立菱形，没有与旧六点伪连线。图形支持上述机制判断，但尚未执行候选冻结。
- STOPPED_AS_DESIGNED：资源守护在完成后正常退出，当前无细化、后处理或开发阶段进程，GPU空闲。没有自动启动8动作开发、读取独立确认集或渲染视频。

### Immediate continuation

1. 审读v2.4权衡图与配置汇总，明确M6是否同时保留 `scan_05` 和 `scan_07`；M3若进入开发，只能标记为探索性负对照，不能称成功候选。
2. 使用已验证的冻结器创建开发前冻结文件；每方法最多两组，角色和配置写入哈希。冻结前不得启动8动作×3种子开发扩样。
3. 开发完整后做不重新选参的描述性分析，再创建确认前冻结；随后运行隔离的4动作×3种子确认，最后才生成固定侧视七剂量MP4。

## 2026-09-16：细化继续运行，自动后处理入口已验证

### 已验证事实

- IN PROGRESS：2026-09-17 00:15（服务器 CST）资源门连续两次通过后，`candidate_refinement_resource_guard_v5` 已启动一次性技术续跑。M3 `scan_07`、seed0、−10° 的 `attempt_02` 使用与失败尝试完全相同的协议哈希 `2d9065…74f`、配置哈希 `2768fe…57e17` 和参数，在约97.4秒完成生成及即时评价；`attempt_01` 原样保留。有效完成进度为 `42/112`：M1 `scan_07/08` 和 M3 `scan_07` 均为14/14批，M3 `scan_08` 已开始；总 `progress.json` 含一个资源中断记录，禁止用全部记录数代替 `COMPLETED` 唯一键计数。
- VERIFIED/INTERIM：M1 两组细化的112条即时控制评价完整。低裁剪阈值 `0.1` 的 `scan_08` 在−10°把角度通过数由 `3/8` 提高到 `6/8`，中位角度误差由约 `0.984°` 降至 `0.389°`；±2°、±5°和+10°未见明显控制损失。这证明裁剪参数在本次低阈值范围内会改变结果，但最终配置仍必须等待新版物理与内容评价共同决定。
- VERIFIED/INTERIM：M3 `scan_07`（单步上限4°、最多2次投影）14/14批和56条即时评价完整，非有限值0，8条0°均逐位恒等；48条非零剂量全部未过既有角度门，±10°中位角度误差约 `9.93°`。该完整负结果保留；尚未完成的 `scan_08` 用于检验把最多投影次数提高到8是否能恢复控制。
- IMPLEMENTED/VERIFIED：新增 `experiments/postprocess_candidate_refinement_v2.py`，只在细化阶段数据链满足 112 批、448 条生成动作、8 条 M0、56 条 M7、504 条评价且失败为 0 时运行。它依次执行机制诊断、严格合并旧六点与细化评价、绘制 v2.4 图并输出开发候选建议；结束状态固定为 `COMPLETE_NOT_FROZEN`，不会自动冻结候选或读取独立确认结果。
- FIXED/VERIFIED：直接执行后处理脚本原先因仓库根目录不在 `sys.path` 而在导入 `experiments.*` 时失败；现已增加脚本入口路径引导和子进程回归。随后又补齐旧六点与细化记录的严格合并及 M4 机制覆盖；当前本地与服务器相关回归均为 `16 passed`。
- VERIFIED：预注册执行协议已包含隔离的 8 个开发动作、4 个独立确认动作及对应清单。运行器的开发阶段会复用四动作×两种子已有结果，只补另外四动作×两种子及全部八动作的 seed 2026；独立确认仍由开发完整性和确认前冻结文件双重硬门保护，无需改变数据划分。
- FIXED/VERIFIED：细化自动后处理的机制诊断输入已改为严格合并旧六点与细化两份运行记录清单，避免只读 `refine.json` 时漏掉没有新增配置的 M4。后处理会强制要求 M1/M3/M4/M5/M6 全覆盖；本地与服务器相关回归均为 `16 passed`。
- IMPLEMENTED/VERIFIED：新增 `experiments/analyze_frozen_candidate_stage.py`，用于 8 动作开发和 4 动作独立确认的冻结候选描述性比较。它验证完整动作—种子—剂量矩阵、冻结后参数、零剂量恒等、数值稳定、M2 阻断和共同支持，输出跨候选总览、每方法七剂量响应及源数据；明确 `selection_performed=false`、`confirmation_used_for_tuning=false`。合成端到端回归能实际导出 PDF/SVG/PNG；服务器冻结/运行/视频组合测试为 `21 passed`。
- VERIFIED：按每方法一到两组冻结候选动态计数：五种生成方法均一组时，开发阶段为 175 个生成批次、840 条生成动作；均两组时为 350 批次、1,680 条。确认阶段相应为 105–210 个生成批次。四动作×两种子的旧格点只在所有哈希一致时复用，不重新采样。
- VERIFIED：2026-09-16 22:12 M1 两组细化共 `28/28` 批全部完成；最后一个 `scan_08`、seed42、+10° 批次因三个外部 `CD4_E3_DSPIN` 拟合组重新启动而从约100秒延长到约14分钟，但四动作在日志中持续推进并最终完整登记，没有失败或择优重跑。外部任务占用大量 CPU 而非本项目 GPU，严禁为提速擅自终止。
- RESOURCE_INTERRUPTED：2026-09-16 22:26，M3 `scan_07`、seed0、−10° 的 `attempt_01` 在进入真实采样前被系统 `SIGKILL`，子进程返回码 `-9`；运行器按首个失败停止并在 `progress.json` 记录 `STOPPED_ON_FAILED_JOB`。容器 cgroup 同时显示 `oom=0`、`oom_kill=0`，不是项目数值失败或容器内存越界；当时 8 个外部 `CD4_E3_DSPIN/fit_directed_gene_network.py` 子进程占用大量 CPU 和内存，`memory.current` 约49.8 GiB。失败目录与运行记录保留，不能计入完成或算法失败率。
- IN PROGRESS：经测试的单次资源恢复守护 `candidate_refinement_resource_guard_v5` 已通过两次资源检查并进入续跑；代码为 `experiments/resume_candidate_refinement_after_resources.py`，本地和服务器专项均为 `8 passed`。只有112批与504条评价完整后才运行新目录 `candidate_refinement_postprocess_v2_4_20260916_attempt_01/`，不会冻结候选、启动开发或读取确认数据。日志为 `candidate_scan_runs_v2/refine_resource_guard_v5_20260916.log`。
- VERIFIED：确认冻结器增加执行顺序硬门：开发冻结必须来自 `DEVELOPMENT_CANDIDATES_AFTER_REFINEMENT`；确认前冻结必须同时看到完整开发摘要和 `COMPLETE_DESCRIPTIVE_NO_RESELECTION` 的开发分析，并保持所有候选配置和角色不变。服务器冻结、开发分析和视频组合回归为 `19 passed`。

### Immediate continuation

1. 只读监控 `candidate_refinement_resource_guard_v5` 及其日志；当前有效完成为 `42/112`，M1和M3 `scan_07` 已完成，M3 `scan_08` 正常续跑。若再次失败，守护停止且不得循环重试。
2. 达到 112/112 后核验 504/504 评价与所有来源哈希，再运行版本化后处理入口；不得覆盖 v2.3 或旧六点结果。
3. 人工复核 v2.4 图和机制诊断后冻结每方法最多两个开发候选，再启动 8 动作×3 种子开发扩样。
4. 开发完整后冻结候选配置与角色，随后运行隔离的 4 动作×3 种子确认；确认完成后才生成固定侧视七剂量 MP4。

## 2026-09-16：细化协议冻结，资源恢复后冒烟通过

### 已验证事实

- VERIFIED：预注册细化执行协议已生成并在新动作生成前冻结，正式本地目录为 `diagnostics/candidate_refinement_protocol_v2_20260916_attempt_03/`，服务器目录为 `/root/autodl-tmp/vimogen_m1_m7_scale/results/phase9/pelvis_m1_m7/candidate_refinement_protocol_v2/attempt_03/`；两端 `scan_protocol.json` 的 SHA256 均为 `2d9065b129bc09dace624b94579c818fbbf19498d232a5245466f29021edd74f`。源配置模板仍保留 `PROPOSED_NOT_FROZEN`，实际自包含执行协议明确为 `FROZEN_BEFORE_REFINEMENT_GENERATION`，不得混淆二者状态。
- VERIFIED：细化新增点为 M1 `scan_07/08`、M3 `scan_07/08`、M5 `scan_07/08`、M6 `scan_07/08`；M4 不新增 16° 信赖域点，只做旧六点终端修正前后诊断；M2 继续因批量—单样本不变量失败而阻断；M7 继续作为生成后编辑参考。每种方法仍满足最多八组配置的预算。
- VERIFIED：M1 采用引导强度 `0.0353553391`，分别比较裁剪阈值 `1.0/0.1`；M3 采用单步最大修正 `4°`，分别比较最多投影 `2/8` 次；M5 比较惩罚增益 `0.2828427125/0.4`；M6 比较候选修正强度 `1.4142135624/2.0`。这些点来自机制诊断和内部几何中点，不是机械扩大所有边界。
- VERIFIED：服务器旧网格机制诊断位于 `candidate_refinement_diagnostics_v1/grid_baseline/`，覆盖 1,680 条生成方法序列、30 个配置、缺失 0，且 `selection_performed=false`。M1 旧网格裁剪前梯度 RMS 最大约 `0.182949`，阈值 `0.5/1.0` 均未触发；M3 单序列最多实际投影 2 次且存在投影预算耗尽；M4 终端修正前残差中位约 `4.90°`，修正后多数配置接近数值零。
- VERIFIED：2026-09-16 服务器相关回归重新执行为 `28 passed`；冻结协议干跑仍得到 112 个新增生成批次、448 条动作序列、已记录 0，M2 保持阻断，M4 无新增配置。
- VERIFIED/HISTORICAL_BLOCKER：首次 M1 `scan_07`、seed 0、−10° 冒烟没有进入 GPU 推理，约 9 分钟后为避免与无关任务争用内存而人工终止。失败证据保留在 `candidate_scan_runs_v2/runs/M1/scan_07/seed_000/dose_-10/attempt_01/`；没有生成新动作，也没有在 `progress.json` 登记完成。其 `run_record.json` 仍是未完成的 `RUNNING` 快照，未覆盖或删除。
- RESOLVED：2026-09-16 20:53（服务器 CST）三个 `/root/autodl-tmp/CD4_E3_DSPIN` 外部任务全部结束，匿名内存降至约 7.73 GB。资源守护只启动一个预定冒烟批次，没有自动启动完整细化。
- VERIFIED：M1 `scan_07`、seed 0、−10° 的 `attempt_02` 已完成四动作真实生成和即时评价，采样阶段约 98.2 秒，守护总退出码为 0；阶段进度为 `1/112`，其余 `111` 批次尚待运行。协议哈希、配置哈希和四组 M0/源噪声哈希均正确，四条记录非有限值计数均为 0；角度门为 `1/4`，该真实负例按协议保留，不择优重跑。
- VERIFIED：该冒烟真实推理显存约 9.73 GB。M1 诊断包含 72 个有效引导步，裁剪前梯度 RMS 为 `0.004588–0.226740`，接受尺度均为 1，拒绝步和非有限值均为 0；`gradient_clip_rms=1.0` 未触发符合预期，证明后续 `scan_08=0.1` 可作为有效裁剪敏感性对照。
- VERIFIED：首个 M1 `scan_08`、seed 0、−10° 批次已完成生成，设置中的 `gradient_clip_rms=0.1` 与协议哈希正确；72 个有效步中 42 个裁剪前 RMS 大于 0.1，非有限值 0。真实触发步的状态更新 RMS 上限为 `0.0035355`，与实现公式 `guidance_scale 0.035355 × clip 0.1` 一致，说明本次低阈值对照确实激活裁剪；`accepted_scale=1` 是线搜索尺度，不是裁剪比例。
- IN PROGRESS：2026-09-16 21:00（服务器 CST）已在后台会话 `candidate_refinement_v2` 启动剩余细化批次的单路断点续跑，日志为 `candidate_scan_runs_v2/refine_full_20260916.log`。运行器已正确复用首批 `scan_07/seed0/−10°`，从 `scan_07/seed0/−5°` 继续；任何生成失败都会按现有停止规则退出，不自动择优重跑。
- IMPLEMENTED/VERIFIED：新增 `experiments/merge_candidate_metric_tables.py` 与专项测试，以方法—配置—动作—种子—剂量作为唯一键合并旧六点和细化评价；重复记录只允许来源路径不同，动作哈希、配对 M0、控制、物理、内容及支持诊断必须一致，否则停止。真实 1,736 条自合并检查得到 3,472→1,736 行、严格去重 1,736 条。
- IMPLEMENTED/VERIFIED：统一图分析器升级为图协议 v2.4。原六点仍以线表示，预注册细化点只以带配置编号的独立菱形表示，避免 M3 相同主参数但不同投影预算被伪连成连续趋势；增加 `--analysis-stage refinement`，其选择状态明确为细化后的开发候选。旧正式 v2.3 目录与图不覆盖。
- IMPLEMENTED/VERIFIED：冻结器现在同时保留新版 `control_priority_reference` 与 `comprehensive_candidate` 角色，避免沿用旧低穿地/低滑动字段时丢失角色。服务器合并、分析、冻结与运行器专项测试为 `16 passed`；真实六点 v2.4 临时回归生成 16 组图，选择编号与正式 v2.3 一致。以上只是分析链准备，不代表细化选参已经完成。
- IMPLEMENTED/VERIFIED：新增 `experiments/render_candidate_side_videos.py`，视频生成具有独立确认完成硬门：必须提供 `STAGE_DATA_CHAIN_COMPLETE` 的 confirmation 摘要和 `FROZEN_BEFORE_CONFIRMATION` 冻结候选，M2 阻断时不可作为候选，七档剂量必须完整且数值有效。输出使用配对 M0 决定的共享固定矢状面相机，M0 与候选并排，显示方法、配置和目标角度，并记录动作/M0/视频/渲染器哈希。
- IMPLEMENTED/VERIFIED：通用化 `scripts/render_absolute_mean_triptych.py` 的源渲染接口，允许任意候选键名和真实帧率，同时保持旧 M0/G0/G1 主入口兼容。权威运行时原脚本已备份；视频硬门测试 `4 passed`，运行时固定相机测试 `2 passed`。未生成正式视频，符合“确认后再看视频”的冻结顺序。

### 决策与不变量

- DECISION：单批冒烟已满足四动作生成、即时评价、协议哈希和梯度诊断退出条件；可以用相同冻结协议和断点续跑机制启动其余 111 个细化批次。
- DECISION：细化结果必须追加到现有 `candidate_scan_runs_v2` 的新尝试目录；旧六点网格、正式统一图 v2.3、Physical V2 和所有失败尝试均不得覆盖或删除。

### Immediate continuation

1. 监控后台会话 `candidate_refinement_v2` 和 `refine_full_20260916.log`，直至剩余 111 个细化批次完成或首个失败；逐配置保存并记录内存、显存和进度，不得覆盖 `attempt_01/02`。
2. 全部批次完成后补算新版内容与物理指标、运行机制诊断、合并旧六点和新增点并重绘参数图。
3. 每方法最多冻结两个开发候选，再按 8 动作×3 开发种子扩样；参数和角色冻结后才读取独立确认集，之后生成固定侧视不同角度 MP4。
4. 仅当公平比较仍显示共同失败时，另立版本开展接触协调补偿新方法原型；不得包装为原方法普通调参。

## 2026-09-16：新版统一图 v2.3 完成，进入预注册细化

### 已验证事实

- VERIFIED：`experiments/analyze_m1_m7_candidate_scan_v2.py` 已完成第二轮图形修订，正式输出位于 `diagnostics/candidate_grid_analysis_v2_3_20260916/`。输入仍为同一份 1,736 条新版评价记录，共 31 个配置、16 组图、48 个 PDF/SVG/PNG 文件；源数据未删减，v2 与 v2.3 的方法选择对象逐字一致，`selection_changed_from_v2=false`。
- VERIFIED：参数响应图的穿地面板改为动作等权的最大穿地深度和穿地帧率，不再用全零的配置中位 P95 制造伪精细坐标；控制—物理权衡图也以最大穿地深度替代 P95，但这只是图形展示修订，没有修改 v2 选择规则。
- VERIFIED：七剂量图的纵轴现在直接由所选配置全部七档逐序列观测计算，M4 的 ±10° 近地速度点不再被裁剪；多配置折线与散点使用同一横向偏移，消除了穿地面板伪竖线。动作编号—颜色和种子—点形均有显式图例。
- VERIFIED：M1 的 `gradient_clip_rms=0.5/1.0` 曲线在所有展示指标完全重合时只画一条合并曲线，并注明重合，解决“图例有蓝黄两色但画面只有一色”的歧义。M6 等权衡图的同横坐标标签采用向坐标区内部堆叠避让。
- VERIFIED：绘图专项测试 `6 passed`；科研图源码预检 `12 PASS / 0 FAIL / 2 WARN`，两项警告仅为没有 TIFF、PNG 为 300 dpi，而既定交付要求是 PDF/SVG/PNG。M1、M4、M6 代表性最终尺寸 PNG 已人工检查。扩展本地测试因本机 Python 缺少 `torch` 在收集阶段中止，模型相关回归仍以服务器环境为准。

### 决策与不变量

- DECISION：`candidate_grid_analysis_v2_3_20260916` 是当前正式可视化目录；`v2_1`、`v2_2` 作为未删除的绘图迭代证据，不得误当正式结果。原 `candidate_grid_analysis_v2_20260916` 仍保留，不覆盖。
- DECISION：图形修订不构成重新选参。当前探索性选择仍为 M1 `scan_05`、M3 `scan_05`、M4 控制参考 `scan_05` 与综合候选 `scan_02`、M5 `scan_06`、M6 `scan_05`；M2继续阻断，M7继续作为生成后编辑参考。

### Immediate continuation

1. 建立新版预注册细化协议，显式列出每方法至多两个新增配置及诊断要求；不得继续使用旧 `refine_candidate_scan_protocol.py` 的机械边界翻倍输出。
2. 在服务器 `mdm5090` 环境运行分析器和相关测试，按文件清单同步；然后只运行预注册新增点，不覆盖六点扫描。
3. 新版离线评价细化结果并重绘，最多保留两个开发候选；再扩至8动作×3开发种子。
4. 冻结参数、角色和暂定推荐后才读取独立确认集，之后生成固定侧视walk视频。

## 2026-09-16：新版内容评价完成，参数细化准备中

### 已验证事实

- VERIFIED：新增独立评价器 `evaluation/content_metrics_v2.py`，旧 `content_metrics.py` 字节保持不变，因此旧生成代码指纹和历史结果仍可复用。新版报告根局部21关节位置偏差、腿/脊柱/手臂局部旋转SO(3)偏差、根轨迹、世界躯干方向及按真实帧率换算的局部速度/加速度/急动度偏差；原始相对M0全身MPJPE只保留历史溯源，不进入新版选择。
- VERIFIED：合成评价回归在服务器通过 `5/5`；相关物理与离线重评回归通过 `20/20`。真实冻结M0上的八项闸门全部通过，包括接触恒等、穿地/滑动/失去支撑扰动、内容恒等、平移隔离、纯根旋转隔离和局部腿旋转检测。SO(3)恒等残差使用双精度 `atan2(sin, cos)` 修复，避免单精度 `acos` 虚报约0.03°。
- VERIFIED：真实分层小试覆盖M1、M4、M7共3条0°记录，完成 `3/3`、失败0，19个v2字段完整且全部恒等指标为0。正式六点扫描新版离线补算完成 `1736/1736`、失败0，没有生成新动作，旧 Physical V2 未改变。
- VERIFIED：新版产物位于服务器 `candidate_scan_runs_v2/candidate_evaluations_v2_grid/`；`candidate_metrics.csv` SHA256为 `0483a29a3036152f08375f36798610c431daf365c87b1be758adf80fee1b5dbc`，记录分解为M1/M3/M4/M5/M6各336条、M7 56条。
- VERIFIED：M2既有重复批审计继续为PASS，但批量—单样本一致性v1/v2均为FAIL，因此M2仍被阻断，尚未进入参数扫描。该状态没有因单元测试通过而改变。

### 当前执行顺序

1. 用新版指标建立v2选择器与统一参数图，旧v1图和选择文件原样保留。
2. 完成M1/M3/M4/M5/M6的预注册有界细化及M4终端修正前后诊断；M2继续阻断。
3. 每方法保留控制优先参考与最多一个综合候选，扩至8动作×3开发种子。
4. 冻结参数、角色和暂定主推荐后，才读取独立确认集并生成固定侧视walk视频。
5. 只有公平比较仍显示共同失败时，才启动接触协调补偿原型。

## 2026-09-16：候选物理重评、中心矩阵与六点参数扫描完成

### 已验证事实

- VERIFIED：从 252 条审计记录及补充历史配置建立唯一回放链，恢复 8 条真实历史配对 M0。最终候选物理重评完成 `472/472`、失败 `0`；旧 Physical V2 结果未改写。服务器产物分别位于 `candidate_archived_true_m0_reference_v2/` 与 `candidate_reevaluation_mesh_v3/`。
- VERIFIED：历史参数总览覆盖 M1–M7、48 个配置汇总，导出 7 张图的 PDF/SVG/PNG/TIFF 共 28 个文件及逐序列、逐配置源数据。历史覆盖不等，机器记录明确为 `common_support_complete=false`，因此这些图只能解释旧配置差异，不能选择赢家或构造单参数趋势。服务器产物位于 `candidate_historical_parameter_analysis_v2/`。
- VERIFIED：24 条盲法回放复核索引已生成在 `candidate_reevaluation_mesh_v3/blind_replay_review_v2/`。0° 审计确认新 S1 的 M1/M3–M6 各 8 条逐位恒等；旧 S0 各方法 4 条存在真实漂移；M2 的 4 条均存在真实不变量失败；M7 仅有约 `10^-7` 的历史浮点差。
- VERIFIED：历史图 Python 源码预检 `14/14` 通过；7 张最终尺寸 PNG 人工检查未发现裁切、重叠或伪连线。图中保留失败数、不可评价数、序列覆盖和“覆盖不等，仅散点”提示；M2 只有一个可追溯配置、M7 仅为生成后编辑参考，不人为补点。
- FIXED/VERIFIED：首个中心任务先后暴露两次运行期包加载失败，`attempt_01` 为缺失 `motion_rep.motion_checker`，`attempt_02` 为历史 `sampling` 与检出 `motion_rep` 接口混装；两个失败目录均保留。根因是候选运行器在进入运行根之前通过 `guidance.base` 提前加载了检出目录的 `motion_rep`。现改为从无运行期依赖的 `candidate_scan_provenance.py` 读取配置哈希，恢复旧 S1 的包级加载边界；服务器候选专项测试 `63 passed`。
- VERIFIED：修复后的 M1 中心 `scan_04`、seed0、−10°、4 动作任务在 `attempt_03` 完成，配置 `guidance_scale=0.025`、`gradient_clip_rms=1.0`、`sigma_min=0.0662879`、`sigma_max=0.65` 已在解析配置和逐样本记录中生效。4 条动作各有唯一配对 M0 与源噪声哈希，无非有限值；角度门为 `3/4`，该负例按计划保留，不择优重跑。生成与即时评价共约 `108 s`；显存峰值字段仍为 0，不能作为成本证据。
- VERIFIED：中心阶段已完成 `70/70` 个生成批次、280 条生成方法序列、56 条 M7 和 8 条 M0，总计 `344/344` 条；336 条候选动作物理重评全部成功。共享 M0/源噪声审计和评价器扰动验证均通过，五种生成方法的 0° 输出各 `8/8` 逐位恒等且无非有限值。结构化证据位于 `candidate_scan_runs_v2/progress.json`、`center_summary.json`、`m0_audits/center.json` 和 `candidate_evaluations/center/`。
- VERIFIED：中心控制结果为：M1 在 ±2°、±5°、±10° 分别通过 `16/16、13/16、12/16`；M3 所有非零剂量均 `0/8`，实际响应仅约 ±0.02°，属于中心参数过于保守的有效负结果；M4 七档均 `8/8`，误差接近数值零；M5 分别通过 `16/16、12/16、8/16`；M6 分别通过 `16/16、2/16、1/16`，大剂量明显欠响应。中心结果只用于数据链与起点诊断，不能替代共同支持的六点参数扫描。
- VERIFIED：四动作批次实时显存约 9.3–9.7 GiB、GPU 利用率约 11%–24%、温度最高 42°C；运行器内部显存峰值字段仍错误为 0，禁止用该字段比较成本。
- VERIFIED：中心 336 条候选记录的穿地 P95、近地滑动速度 P95、异常失去支撑率和相对 M0 全身关节平均偏差均无缺失或非有限值。最终中心诊断已输出到服务器 `candidate_center_analysis_v3/`，共 6 个配置、48 个 PDF/SVG/PNG/TIFF 文件；`candidate_selection_performed=false`，各生成方法状态均为 `NOT_APPLICABLE_CENTER_DIAGNOSTIC_ONLY`，因此不会用单一中心点提前选参。`v1` 因中文字体缺失且分析模式边界不严谨、`v2` 因机器可读选择文件尚未显式写入该字段而保留为无效诊断，不作正式证据。
- VERIFIED：最终中心诊断图的 Python 源码预检为 `14/14`，分析专项测试为 `8 passed`；M3 与 M4 的最终尺寸 PNG 已人工检查，无裁切、重叠或缺字。M3 图清晰显示实际响应近零且偏离理想线，M4 图清晰显示控制近乎精确但穿地、滑动仍有明显动作级离散，支持“控制效果与物理副作用必须分开评价”的证据逻辑。
- FIXED/VERIFIED：首次网格后台任务误用基础 Python，因缺少 `omegaconf` 在首任务进入模型前失败并按协议停止，失败记录保留且未生成候选动作。权威项目记录确认动态运行环境为 `/root/miniconda3/envs/mdm5090/bin/python`；使用该解释器后，M1 `scan_01`、seed0、−10° 四动作任务完成，解析设置为 `guidance_scale=0.0125`、`gradient_clip_rms=0.5`，与冻结协议和配置哈希一致。
- VERIFIED：六点网格于 2026-09-16 13:36（北京时间）核验完成，后台会话和生成进程均已正常退出。最终完成 420 个生成批次、1,680 条生成方法序列、56 条 M7 和 8 条 M0，共 1,744 个序列任务；1,736 条候选动作全部完成新协议评价，失败 0。八个动作—种子配对的共享 M0 与源噪声审计通过，评价器恒等、穿地、滑动和失去支撑扰动验证均通过；正式 Physical V2 未改变，新候选阈值仍未冻结。
- VERIFIED：正式六点扫描分析输出在服务器 `candidate_grid_analysis_v1/`，汇总 31 个配置、6 种方法和 1,736 条逐序列记录，共导出 27 张图的 PDF/SVG/PNG/TIFF 108 个图形文件及源数据；所有生成方法的六组配置均具有共同动作—种子支持。代表性最终尺寸 PNG 已人工检查，无裁切、重叠或缺字。
- VERIFIED：探索性选参结果为 M1 `scan_06`（核心角度通过率 93.75%，±10° 31.25%）、M3 `scan_06`（0%，0%）、M4 `scan_05`（100%，100%）、M5 `scan_06`（87.5%，50%）、M6 `scan_05`（90.625%，50%）。五个探索候选的主参数均位于扫描上边界，因此只能称为“已测试范围内优选”，不冻结参数也不声称全局最优；M3 仍属于控制失败，只有 M4 达到完整控制。M1 的 `scan_05/06` 汇总完全相同，说明当前梯度裁剪未形成可见区分；M4 在信赖域 4° 与 8° 时控制差异仅为约 `10^-7` 度量级，边界最优主要来自数值级排序，继续扩张的科学优先级低于 M1/M5/M6。
- VERIFIED：M1 参数图中蓝色与黄色曲线并非漏画。`gradient_clip_rms=0.5/1.0` 的三对配置各 56 条动作均为动作哈希逐位相同、绘图指标完全相同；运行记录确认两个阈值均实际传入。全六配置、每配置 864 个有效引导步的裁剪前梯度 RMS 最大值均为 `0.2294815928`，没有一步超过 0.5，因此两阈值从未触发，后绘制的黄色曲线完全覆盖蓝色。当前次参数扫描属于非信息性范围，不能据此判断梯度裁剪无效；后续应先用观测范围内的阈值做预注册敏感性小试，或在图中合并重合曲线并明确标注“裁剪未触发”。
- VERIFIED：剂量响应图中的彩色空心散点是单条动作—种子序列；颜色没有预定义科学含义。绘图器对每个剂量重复调用未指定颜色的 `scatter`，触发 Matplotlib 自动色轮，因此颜色随绘制顺序变化，容易被误读为动作或种子类别。近地速度高点主要来自 sample34122，其配对 M0 已为约 `1101–1382 mm/s`；M3 高点相对 M0 的变化约为 `−58` 至 `+51 mm/s`，主要继承基线。M4 除继承基线外存在明确增量，最大为 sample94、seed0、−10° 的 `+525.5 mm/s`。正式图应固定动作颜色/种子形状，并同时显示绝对值和相对配对 M0 的增量。
- VERIFIED：服务器 `mdm5090` 环境的候选专项测试为 `64 passed`；本地图形分析专项测试为 `8 passed`、无模型依赖测试为 `17 passed`。本地完整候选测试无法收集是因为本机 Python 缺少 `torch` 和 `omegaconf`，不是测试断言失败；模型依赖测试以服务器结果为准。项目记忆检查和 `git diff --check` 均通过。

### 决策与不变量

- USER DIRECTION（2026-09-16）：用户不接受原始全身MPJPE继续作为关键指标或主要选参依据，要求能区分目标编辑与非目标改变；旧数据保留溯源。用户提出21个非根关节比较、独立阈值校准，并要求配置确认后生成不同角度的侧视walk MP4检查自然性。新选择器/图形尚未修改，视频尚未生成。
- VERIFIED/PENDING：FK显示根旋转传递到全部后代，纯根旋转且根中心固定时，去掉根关节后的原始MPJPE=22/21倍全身MPJPE，不能消除混杂。建议试验根局部坐标21关节偏差并结合局部旋转、世界躯干、根轨迹及接触指标，仅作诊断直至独立验证。已有 `scripts/render_absolute_mean_triptych.py:86` 的共享M0固定侧视相机可复用，但需接入当前M1–M7归档、真实fps及配置标识。

- VERIFIED（2026-09-16，控制—保持定向复核）：已下载并分析1,736条扫描记录，源哈希与配对计数保存在 `diagnostics/control_content_audit_20260916/`。M1 0.0125→0.025的32个核心剂量配对全部MAE下降、MPJPE上升；0.025→0.05则20/32两项一起变差，实际剂量出现过冲。MPJPE是相对M0的世界坐标22关节总偏差，混入目标根旋转及必要补偿，不能独立称为非目标内容损失。M7纯根编辑的MPJPE约13.494/33.725/67.387 mm（±2/5/10°），提供几何参考但不是真值或下界。
- VERIFIED：原参数图为核心剂量的“动作内中位数，再跨动作中位数”。992条核心记录（含重复配置）中131条穿地P95>0、849条最大深度>0，最大23.105 mm；配置中位汇总为0不代表没有穿地。当前选择器在物理比较前按最高控制通过率及最小MAE+1e-12筛选，可能使单条通过差异或数值精度主导候选；这是原规则结果，未擅自修改。
- RECOMMENDATION/PENDING：优先离线补做根平移、根局部形状、局部关节旋转及世界躯干/接触诊断，再讨论细化。暂缓机械按边界翻倍，尤其M1已过冲、M4已近乎精确。详见 `CONTROL_CONTENT_TRADEOFF_REVIEW_20260916.md`，已核对GMD、OmniControl、DNO、MotionFix原文；新增诊断和选参修改均未实施、须新版本协议，不覆盖旧数据。

- DECISION：新候选物理协议只报告穿地、近地滑动、适用情境下的异常失去支撑及支撑高度诊断等连续量；独立校准前不宣布新协议物理通过。正式 Physical V2 历史口径继续冻结，支撑高度不删除，但不作为新候选协议中的独立物理否决项。
- DECISION：每种方法的参数变化图是选参必要证据；新扫描只能在共同动作—种子支持上称“已测试范围内优选配置”。若优选点落在边界，必须标记仍有扩展空间。
- FROZEN：主要控制范围为 ±2°、±5°；±10°作为次级稳健性比较；0°单列检查不变量。M2 在不变量修复前继续阻断，M7不新增算法参数扫描。

### Immediate continuation

0. 先阅读 `CONTROL_CONTENT_TRADEOFF_REVIEW_20260916.md` 和离线配对审计。下述原边界翻倍建议暂缓：优先核验内容指标的目标变化混杂，完成同剂量回放，明确新版实质等效规则后再定新增配置；本次未运行新生成或重新冻结参数。
1. 先修订正式图形规范：跨方法同类图固定坐标范围；剂量响应散点使用固定动作颜色和种子形状；近地速度同时画绝对值及相对配对 M0 增量；M1 完全重合的裁剪曲线合并并标注裁剪未触发。旧图保留为诊断版本，不覆盖。
2. 按预定边界规则建立细化协议：每种生成方法先增加一个主参数翻倍点，建议 M1 引导强度 0.10、M3 单步最大修正 4°、M4 信赖域 16°、M5 惩罚增益 0.40、M6 候选修正强度 2.0；次参数固定当前探索候选值。M1 若继续研究梯度裁剪，应先增加独立的小规模敏感性检查，阈值必须落入已观察梯度 RMS 范围并在运行前冻结。生成前必须检查实现参数域、配置哈希和重复点，并明确 M4 的扩张主要用于排除边界假象。
3. 细化仍使用当前四动作、种子 0/42 和七档剂量的共同支持。若 M3 在 4° 仍无有效响应则停止其参数扩张并转入结构诊断；若 M1/M5/M6 的控制改善伴随内容或物理副作用继续上升，则保留内部点与边界点作为两种角色，不追求单一赢家。
4. 重跑正式扫描分析并冻结每方法最多两组开发候选；随后只把这些候选扩展到八动作、三个开发种子。参数和候选角色在读取四动作独立确认集前冻结，不得用确认结果重新选参。
5. 修复显存峰值采集或继续保留独立实时监控记录；不得把当前 0 值用于成本比较。
