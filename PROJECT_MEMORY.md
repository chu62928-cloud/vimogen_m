...
## 2026-09-06：v0.4.2 B1 接触掩码与残差门修正（待服务器重跑）
...
- 历史 B1 第0帧约`3.31 mm`的失败仍需在掩码修正后重新判断。原型此前在所有骨盆活动帧固定四个足部标记，并以总残差约`3.46 mm`提前停止，故历史失败不能直接归因于解剖自由度不足。

### 待执行事项

- 将修正版代码推送服务器，在独立新 attempt 中重跑 `sample94 / position_only_medium / +2° / dead-zone 0.5°`，保存每侧平足证据、每标记点残差、冻结地面轴和完整源哈希。
- 若 B1 仍失败，仅对失败帧运行单脚／脚跟／脚尖／双脚及根平移`1/5/10 mm`诊断；未完成前不进入 B2、B3、C，不提高剂量。
