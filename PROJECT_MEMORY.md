...
- S1 屏选已完成 `40/40` 组并生成选择文件；M1、M3、M4、M5、M6 的屏选前两名均为配置 `01、02`，M2 因不变量失败跳过。M1、M4 的屏选配置均角度 `4/4`但物理 v2 `0/4`；M3 均角度 `0/4`；M5 只有较高增益配置角度 `4/4`；M6 屏选组角度均 `4/4`但物理 v2 `0/4`。确认阶段已重新启动，首次自动串联失败原因是后台命令遗漏 `PYTHONPATH=.`，未造成结果覆盖。
