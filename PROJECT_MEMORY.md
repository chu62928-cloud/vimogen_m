...
## 2026-09-10：v2 方法级物理通过数复核完成

### 已验证事实

- 复核服务器机器可读汇总后确认，v2 总数 `22/84` 正确；方法级分解为 M1 `4/12`、M2 `2/12`、M3 `0/12`、M4 `0/12`、M5 `8/12`、M6 `5/12`、M7 `3/12`，合计为 `22/84`。
- 原先的 `0/12、2/12、0/12、0/12、2/12、2/12、2/12` 是将部分 v1 方法级数字误抄到 v2 列，已同步修正 `result.md`、`README.md` 和本文件；服务器原始结果未改变。
- 新增 `tests/test_result_ledger.py` 文档算术一致性回归测试，修正前按预期失败（`8 != 22`），修正后已通过。

## 2026-09-09：S1 有界筛选、压力测试与冻结完成

### 已验证事实

- 当前分支已恢复论文权威账本 `result.md`，集中维护 M1–M7 的 S0/S1 数值表、结论边界、复现产物与更新记录；今后实验输出、冻结状态、结果表或已验证结论变化时，必须在同一变更中更新该文件。
...
- 84 条 S0 序列已完成双轨物理评价：v1 `7/84`、v2 `22/84`。v1 方法级为 M1 `0/12`、M2 `2/12`、M3 `0/12`、M4 `0/12`、M5 `2/12`、M6 `2/12`、M7 `1/12`；v2 方法级为 M1 `4/12`、M2 `2/12`、M3 `0/12`、M4 `0/12`、M5 `8/12`、M6 `5/12`、M7 `3/12`，合计 `22/84`。双轨 Table 1、84 行附表和热图位于服务器 `results/phase9/pelvis_m1_m7/table1_s0_physical_v2/attempt_01/`。
...
- M7 的 2 样本 × 2 种子 × 7 剂量参考已生成 28/28；物理复评为 v1 `1/28`、v2 `3/28`。该结果保留为生成后编辑参考，不参与生成期配置排序。
- S1 屏选、确认和压力测试已分别完成 `40/40、10/10、5/5` 个配置摘要，并生成 `S1_FROZEN.json` 与四份最终报告；服务器无残留生成进程。唯一冻结配置为 M1 `03`、M3 `05`、M4 `06`、M5 `05`、M6 `02`，对应角度/物理 v2 通过数分别为 M1 `17/20、5/20`，M3 `4/20、19/20`，M4 `20/20、4/20`，M5 `16/20、5/20`，M6 `12/20、7/20`；所有确认配置数值稳定均 `20/20`。最终状态为 M1/M3/M5/M6 `S1_FAILED_ANGLE`、M2 `S1_FAILED_INVARIANT`、M4 `S1_FAILED_PHYSICAL`，没有 `S1_PASS`。
- S1 有界筛选从服务器独立检出 `/root/autodl-tmp/vimogen_m1_m7_scale/code_checkouts/m1_m7_s1_latest` 的提交 `57a09339e5c9702f91180db9513a358700883bf7` 执行；报告生成器为提交 `1fb079e` 的 `build_s1_reports.py`。服务器专项回归为 `43 passed`。S1 输出根目录为 `results/phase9/pelvis_m1_m7/s1/`，报告状态为 `S1_REPORTS_COMPLETE`，`s2_allowed=false`；按方案停止在 S1，S2 未启动。

### 待执行事项

1. 保留 S1 冻结结果、失败案例、压力测试和复现清单，不追加无界配置。
2. 按方案停止在 S1，不启动 S2；若未来继续，必须先建立新版本冻结方案和新的门控理由。
...
- 当前实施分支为 `codex/pelvis-m1-m7-scale-experiments`，源方案为
  `ViMoGen_M1-M7_规模实验实施方案_20260907.md`，方案 SHA256 为
  `4402ccaa7abbd60f5a06da17fc3907ba77ffce0b3a41040ee8dfd205f4d8d88a`。
- 已建立统一顶层请求、结果、诊断和逐样本严格 JSON 契约；C0–C3 名称、7档剂量、S0 样本/seed、
  角度硬门和每方法8组等价调参预算已经写入 `configs/m1_m7/`，冻结脚本拒绝覆盖已有协议。
- 公共测量层已统一局部矢状骨盆角、权威动作重建、M0地面/接触证据、躯干方向、控制精度、物理副作用和
  内容保持指标。接触与地面证据只从配对M0冻结，目标角曲线不会从引导结果重新定义。
- C0机制核心已实现：M1采样状态损失梯度、M2完整确定性轨迹反传并仅优化源噪声、M3单次局部投影、
  M4预定时刻前向射击与终端高斯—牛顿、M5无显式投影/伪逆的原始—对偶联合更新、M6李雅普诺夫
  伪投影、M7生成后几何编辑。M7明确记录不声称产生生成器自然响应。
- `FlowSampler` 已接入统一采样钩子、下一噪声尺度、批不变切片和可选终端投影边界；旧M0默认路径不启用。
- 服务器 `/root/autodl-tmp/vimogen_m1_m7_scale` 专项测试为 `23 passed`；补入只读冻结回归资产后，
  完整回归为 `323 passed, 1 skipped in 62.81s`，无失败。服务器使用 RTX 4080 SUPER；代码仍由本地推送。
- 提交 `57a689d` 已由本地推送到 GitHub 分支；服务器算法定义已冻结到
  `results/phase9/pelvis_m1_m7/protocol_v1/`。M7 的 S0 子矩阵已在
  `results/phase9/pelvis_m1_m7/s0_m7/attempt_01/` 完成，12/12案例完成且12/12通过角度硬门。
  该批只证明生成后几何编辑的角度命中；因源缓存未含冻结脚部标记，物理指标明确保留为待物化。
- M1 真实 ViMoGen 运行已完成两个独立候选：`guidance_scale=0.01` 的 attempt_01 为 1/2
  角度过门（sample34122 MAE 约1.020°，正式判定失败）；`guidance_scale=0.02` 的 attempt_02
  为2/2过门。两次均为 seed0、sample94/34122、+2°，沿用同一配对 M0 和 source-noise cache；
  attempt_01 的一次评估器边界错误已保留为历史派生目录，正确重算结果在 attempt_01/evaluation_v2。

### 待执行事项

- M1/M3/M5/M6 已有采样钩子，但 M2 的真实 ViMoGen 可微完整轨迹适配器和 M4 的真实剩余轨迹射击适配器
  尚未接入正式实验运行器；在完成适配和真实小样本梯度审计前，不得宣称完整 S0 已开始或通过。
- 完成 M2/M4 的真实 GPU 适配审计，并让 M3/M5/M6 各完成至少一个服务器 GPU 案例，再启动完整84案例 S0；失败案例保留，
  不覆盖、不择优重跑。S0 通过后才进入调参、S2、S3和S4。

## 2026-09-08：M6符号审计与M2标准化边界修正

### 已验证事实

- M6 首次真实 GPU 案例（seed0、+2°、两样本）完成但角度门为 `0/2`；实际剂量约 `-0.24°`，诊断中18次反传均有限，说明是采样尺度递减方向的符号错误而非数值不稳定。
- M6 已在 `guidance/m6_lyaguide.py` 按降噪声尺度方向修正李雅普诺夫导数、候选速度和伪投影符号；提交 `91722a2` 已推送。独立 `attempt_02`（`candidate_scale=0.5`）两样本角度门 `2/2` 通过，未见非有限值。
- M2 首次真实 GPU 案例保留为边界错误失败：可微采样输出标准化张量却被按物理量计算，sample94/sample34122 实际剂量约 `-5.04°/-7.37°`，MPJPE 约 `0.78–0.87 m`。已在 `train_eval_vimogen.py` 增加标准化→权威物理量转换并提交推送 `951d410`；新的 `attempt_02` 正在服务器复跑。
- M2 边界修正后的 `attempt_02`（学习率 `0.01`）方向正确但过冲，sample94/sample34122 实际剂量约 `+3.05°/+3.57°`，角度门 `0/2`；`attempt_03`（学习率 `0.005`）sample34122 通过而 sample94 仍因 P95 和全身偏离失败，当前暂选其作为 S0 诊断参数，不宣称方法通过。
- M4 首次运行保留为批上下文失败；随后修正 M4 禁用批切片、补齐批统计广播及 OmegaConf 容器解析，提交依次为 `4d9b292`、`83e9713`、`89ee3ce`。独立 `attempt_04` 完成并两样本角度门 `2/2` 通过（实际剂量约 `2°`、P95 近零），但 MPJPE 约 `48–74 mm`、根平移 P95 约 `82–95 mm`，作为内容副作用保留。

### 待执行事项

- 已完成并评价 M2/M4 的真实 GPU 适配审计；M2 记录完整轨迹反传计数，M4 记录每个射击步的前向计数。首次失败和修正候选均不覆盖。
- 启动 M1–M7 的完整 S0 矩阵（2 个 seed × 2 个样本 × 3 个剂量，共 84 条样本），逐运行记录配置、状态和评价；在矩阵全部完成及硬门审计前不宣称 S0 通过。

## 2026-09-08：S0矩阵完成、评估修复与回归验证

### 已验证事实

- 服务器 S0 M1–M6 矩阵已在 `/root/autodl-tmp/vimogen_m1_m7_scale/results/phase9/pelvis_m1_m7/s0_sampling/` 完成 36 个唯一批次（2 个 seed × 3 个剂量 × 6 个方法），每批包含 sample94 和 sample34122，共 72 条受控样本；M7 的独立 S0 批次已有 12 条样本，因此 S0 总量为 84 条。
- 矩阵进度文件 `s0_matrix_progress.json` 保留了首次目录覆盖保护失败和后续修复记录；最新唯一键统计为：M1/M3/M4/M6 各 6/6 批次完成角度硬门，M2 为 2/6，M5 为 2/6。M2、M5 未通过的批次均保留逐样本角度与内容指标，未择优删除。
- 6 个历史评估目录通过可恢复运行补齐摘要，物理指标仍明确标为 `PENDING_SHARED_FK_MARKER_MATERIALIZATION`，不能据此宣称地面/接触门通过。M4 各剂量角度命中稳定，但全身 MPJPE 和根轨迹偏差较高；M3 的 seed0/sample94 也出现较大的内容偏差，后续筛选需优先检查内容与物理门。
...
- S0 角度门审计已完成，但 M2/M5 尚未满足入围条件，M4/M3 存在内容副作用，所有方法的物理门待共享脚部标记物化后才能判定；因此暂不进入 S1 调参或 S2 正式比较。
- 下一步应先完成冻结脚部 marker bundle 的物化和物理 evaluator，再按方案中的角度→数值→物理→内容顺序决定是否修正方法或进入 S1；不得用角度通过抵消内容/物理失败。
- 当前 Table 1 只能标记为 `PRELIMINARY_S0_ONLY`；冻结脚部 marker、S1 调参和 S2 规模实验完成前，不得改名为最终论文主表。

## 2026-09-06：v0.4.2 B1 接触掩码与残差门修正及可行性诊断

### 已验证事实

- 在当前 v0.4.2 分支中修正了下肢终端空间原型：正式运行器向求解器传入冻结的左右脚 `flat_contact` 证据和协议 `ground_axis_index`。
- 求解器只对有冻结平足证据的对应侧脚跟／脚尖建立位置方程；每个标记点三维残差、最大标记点残差和逐标记结果均单独记录。
- 求解停止条件改为每个有证据标记点≤`1 mm`，与正式验收定义一致；旧的 `sqrt(12) mm` 总范数停止条件已移除。
- 旧 v0.1–v0.4 采样路径、已有 v0.4 结果和 B1 历史 attempt 未覆盖；本地静态编译和 `git diff --check` 已通过。
- 服务器专项测试在修正后通过 `8 passed`，完整回归随后通过 `304 passed in 49.74s`。
- `attempt_05_masked`（`+2°/0.5°`）的7个骨盆活动帧与冻结平足证据无交集，状态正确记录为 `B1_NOT_EVALUABLE`。
- `attempt_06_exact_masked`（`+2°/0°`诊断）在9个冻结平足证据帧上将最大脚跟／脚尖标记点残差降至`0.0557 mm`，根平移严格为零，最大下肢修正`0.454°`；位置和信赖域通过，但相对投影前穿地增加，整体`B1_FAIL`。
- `feasibility_diagnostic_v2` 完成冻结双脚、单脚、单脚跟／脚尖和根平移上限`1/5/10 mm`诊断。根锁定最大残差约`0.06 mm`；根平移上限`5 mm`时残差约`0.03 mm`、实际最大根增量约`2.80 mm`；`1 mm`上限不足以完全收敛。

### 失败结果与解释

- Windows 本机没有 PyTorch，专项动态测试不能在本机执行；服务器专项测试和 B1 修正版端点尚未重跑，因此当前不能宣称 B1 已通过。
- 历史 B1 第0帧约`3.31 mm`的失败来自旧原型对所有活动帧固定四个足部标记，并以总残差约`3.46 mm`提前停止；掩码修正后该帧不属于冻结平足证据，因此不能把历史失败归因于解剖自由度不足。
- 精确`0°`诊断仍有穿地增加，说明剩余问题是终端骨盆旋转与地面安全约束的冲突；当前还不能进入 B2 前的正式通过状态。

### 待执行事项

- 在独立新 attempt 中保留 `attempt_05_masked`、`attempt_06_exact_masked` 和 `feasibility_diagnostic_v2`，不得覆盖历史结果；诊断 JSON 已从服务器下载至本地结果目录。
- 下一步先根据9个冻结平足帧的诊断结果设计 B2 地面感知求解；在明确“终端相对安全”和“最终绝对接触通过”两层状态前，不进入 B3/C，不提高剂量。
- B1 通过后按 B2→B3→C 顺序推进，并在提高剂量前用 sample34122 验证双脚冻结接触证据。

## 2026-09-06：v0.4.1 A1/A2 与 v0.4.2 B1 已执行

### 已验证事实

- 代码分支：v0.4.1 为 `codex/pelvis-guided-walk-v0-4-1-terminal-dead-zone`，v0.4.2 为
  `codex/pelvis-guided-walk-v0-4-2-lower-body-terminal`；新增终端策略、死区离线评价和根锁定下肢空间原型，已提交并推送。
- A1 只读取 v0.4 sample94/seed0/+2° 的三个投影前端点，共计算12个死区端点；服务器专项测试 v0.4.1/v0.4.2 合计 `11 passed`，远程静态编译通过。
- 当前服务器完整回归为 `301 passed in 52.40s`；未发现新增旧协议回归。
...
- 死区不能解决接触重锚定：`position_only_medium`在所有死区下仍约68–72 mm终端根平移，`temporal_weak`仍约59–62 mm。
- B1 尚未证明根锁定下所有脚部位置都可达；失败可能来自局部轴/目标冲突或地面可行性，而非简单的自由度数量不足。
- sample94右脚平足证据不足，仍为`NOT_EVALUABLE`；本轮不能宣称双脚正式接触通过。生成阶段相对M0的185–202 mm根偏离也未解决。

### 待执行事项

- 不进入B2/B3/C或更高剂量。先审计并改进下肢局部目标、地面不等式和不可行回溯，保持根锁定；重新验证B1最差帧≤1 mm。
- B1通过后加入独立修正平滑项，再加入`temporal_weak`；中等/强接触权重暂不运行。
- 在提高剂量前用sample34122的双脚冻结证据验证；所有正式结果继续与旧v0.4/v0.3分目录保存。
...
- 15 个候选均已在服务器 `videos/` 下生成全身、慢放和足部局部 walk 视频（共 15 个 render_record、45 个 MP4）；大型视频只留服务器结果目录，不进 Git。
...
- VERIFIED：本地静态编译和 `git diff --check` 通过。服务器专项回归在最近三项审计测试加入前已通过 `21 passed`；新增测试已静态编译，但服务器复跑受连接不可用阻塞。本机没有 PyTorch，未把本地动态导入失败当作测试结果。
...
- IMPLEMENTED：新增 `scripts/compare_guided_pelvis_candidates.py`、`scripts/render_guided_pelvis_comparison.py` 和边界测试。输出严格 `comparison.json`、CSV、说明，以及 M0/仅根旋转/v1.3/v2 四栏普通与慢放视频；不修改冻结协议、门限或历史候选。
- VERIFIED：服务器专项测试最终复跑为 `17 passed in 2.55 s`；比较运行完成。视频已实际渲染并抽取第 31/71 帧检查，H.264、1920×1080、20 fps；普通版 100 帧/5.00 s，慢放版 199 帧/9.95 s。
- RESULT/V1.3：剂量 P95 误差 `0.126°`，躯干变化 P95 `0.439°`，关节加速度 P95 `7.439 mm/帧²`、为自身 M0 的 `1.027×`；左足滑 P95 `26.763 mm/帧`。它在躯干与时间自然度上明显优于直接根旋转和当前补偿，但左脚离地/穿地 P95 仍为 `55.156/13.513 mm`，严格接触未通过。
- RESULT/V2：剂量 P95 误差 `1.449°`、躯干 P95 `7.593°`、关节加速度 `9.718 mm/帧²`（`1.303×`）、左足滑 `45.591 mm/帧`、重心代理位移 `234.278 mm`。部分垂向足部指标优于直接根旋转，但整体躯干、足滑、抖动和漂移更差，不能判为总体更自然。
- RESULT/BASELINES：仅根旋转保持时间平滑（关节加速度 `1.021×`）和原水平足滑，但躯干随动约 `10°`，左脚离地/穿地 `80.172/18.724 mm`；当前诊断补偿的关节加速度达到 `11.273×`、躯干 `15.679°`、左足滑 `107.301 mm/帧`，为本次最差候选。
...
- PURPOSE：针对 sample94 步行三栏视频中“诊断补偿比仅根旋转更抖、但两者都前倾”的观察，单独构造只修改冻结 M0 根旋转的 `+10°` 候选，比较足滑/离地/穿地、躯干方向和时间自然度；新增重心/支撑多边形诊断，但不修改 v3.0.1 正式协议或门限。
- IMPLEMENTED：新增 `scripts/build_root_only_motion.py`，仅写入目标根旋转后通过 `authority_project` 统一重建派生通道；`evaluation/pelvis_contact_compensation_v3.py` 新增 `center_of_mass_support_metrics`，以完整 SMPL-X 网格顶点几何中心作为诊断重心代理，在冻结 M0 平足接触帧上计算足底贴片凸包边界距离。重心结果明确标记 `diagnostic_only=true`，没有加入正式 gates。
- VERIFIED：服务器候选生成成功，形状 `100×276`、全部有限；专项测试 `14 passed in 2.27 s`。仅根旋转和诊断补偿均使用同一 M0、协议、SMPL-X 模型和完整网格评价，结果目录分别为 `evaluation_root_only_com_v2/` 与 `evaluation_compensated_com_v2/`，各含 `metrics.json`、自然度 CSV、逐帧 `com_support_per_frame.csv` 和说明，原有 `evaluation/` 未覆盖。
- RESULT/ROOT_ONLY：骨盆剂量 MAE/P95=`0°/0°`；根速度 P95 与 M0 相同 `10.79 mm/帧`，平均关节速度 P95 `20.37 mm/帧`（M0 `19.82`），平均关节加速度 P95 `7.40 mm/帧²`（M0 `7.24`），根轨迹长度保持 `0.659 m`，未出现补偿候选的高频抖动放大。左脚滑动均值/P95=`24.18/28.37 mm/帧`，与 M0 `23.58/29.16` 接近并通过；右脚滑动仅 2 个证据帧对，仍为 `NOT_EVALUABLE`。但仅根旋转导致左/右脚离地 P95=`80.17/80.70 mm`、穿地 P95=`18.72/11.42 mm`，因此足底垂向接触并非严格通过。
- RESULT/TRUNK：仅根旋转躯干方向、骨盆-颈部、骨盆-头部变化 P95 分别约 `10.00°/10.00°/10.00°`；诊断补偿分别约 `15.68°/14.15°/14.86°`。两者均未保持全身直立，诊断补偿更差；水平朝向仍约 `0.020°`，说明问题主要是矢状面前倾而非偏航。
- RESULT/COM：重心代理在 M0 的 5 个稳定平足帧内相对冻结支撑多边形内部比例为 `0%`，故不能把“重心始终在脚内”作为步行动作的硬条件。仅根旋转重心水平位移 P95=`41.2 mm`、相对 M0 支撑边界距离 P95=`−67.3 mm`；诊断补偿为 `91.1 mm`、`−107.3 mm`，全身平衡代理反而恶化。该结论受 sample94 左脚稳定证据少、右脚平足证据不足和几何中心代理限制，只作方向性诊断。
- DECISION：用户关于“接触补偿增加抖动”的判断得到量化支持；当前首要矛盾是固定根剂量引起的全身矢状面前倾，而不是足滑。重心约束值得进入下一轮，但先以稳定支撑帧上的软约束/诊断形式加入，并采用接触置信度渐变；不施加全序列硬性“重心在脚中心”约束。
- NEXT：先修复补偿求解器的时间平滑与阶段 2 可行性保持（线搜索/约束零空间步），再在 sample94 增加重心轨迹和躯干曲线可视化；通过该诊断后才回到 sample34122 的正式双脚窗口。v3.2 仍保持锁定，未训练残差适配器。
- COMMITTED：本轮代码、测试、README 和项目记忆已提交为 `1c11fbf`，并推送到 `codex/pelvis-contact-compensation-walk-diagnostic`；大型运行产物按仓库规则保留在本地/服务器结果目录，不纳入 Git。
