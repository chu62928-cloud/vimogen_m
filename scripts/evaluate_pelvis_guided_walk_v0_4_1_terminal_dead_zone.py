...
def _root_decomposition(m0: torch.Tensor, endpoint: torch.Tensor, pre: torch.Tensor, valid: torch.Tensor, vertical_axis: int) -> dict[str, Any]:
    ...
    return {
        "terminal_increment": {
            "xyz_mm": _stats_signed(terminal[valid], MM),
        ...
        "overall_from_m0": {
            "xyz_mm": _stats_signed(overall[valid], MM),
        ...
        "aligned_shape_deviation": {
            "xyz_mm": _stats_signed(aligned[valid], MM),
...
def _metric_delta(pre: Mapping[str, Any], terminal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, a_path, b_path in (
        ...
        ("terminal_root_translation_p95_mm", ("root", "terminal_increment", "xyz_mm", "norm", "p95"), ("root", "terminal_increment", "xyz_mm", "norm", "p95")),
        ("overall_root_from_m0_p95_mm", ("root", "overall_from_m0", "xyz_mm", "norm", "p95"), ("root", "overall_from_m0", "xyz_mm", "norm", "p95")),
        ("aligned_root_shape_p95_mm", ("root", "aligned_shape_deviation", "xyz_mm", "norm", "p95"), ("root", "aligned_shape_deviation", "xyz_mm", "norm", "p95")),
...
def _classification(pre: Mapping[str, Any], terminal: Mapping[str, Any], delta: Mapping[str, Any], dead_zone: float) -> tuple[str, dict[str, Any]]:
    ...
    for key, limit in (("terminal_root_translation_p95_mm", 1.0),):
        value = terminal["root"]["terminal_increment"]["xyz_mm"]["norm"]["p95"]
        maximum = terminal["root"]["terminal_increment"]["xyz_mm"]["norm"]["max"]
...
def _flat_row(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "terminal_root_delta_p95_mm": terminal["root"]["terminal_increment"]["xyz_mm"]["norm"]["p95"], "terminal_root_delta_max_mm": terminal["root"]["terminal_increment"]["xyz_mm"]["norm"]["max"], "overall_root_m0_p95_mm": terminal["root"]["overall_from_m0"]["xyz_mm"]["norm"]["p95"], "aligned_root_shape_p95_mm": terminal["root"]["aligned_shape_deviation"]["xyz_mm"]["norm"]["p95"],
...
def _run_2x2_if_needed(output: Path, payload: Mapping[str, Any], v04_root: Path, protocol_root: Path, summary_path: Path, model: Any, device: torch.device, m0: torch.Tensor, mean: torch.Tensor, std: torch.Tensor, valid_batch: torch.Tensor, patches: Mapping[str, Any], sides: Mapping[str, Any], m0_joints: torch.Tensor, m0_vertices: torch.Tensor, vertical_axis: int) -> None:
    ...
    for pelvis_enabled in (False, True):
        for contact_enabled in (False, True):
            ...
            rows.append({"pelvis_terminal_enabled": pelvis_enabled, "contact_terminal_enabled": contact_enabled, "root_terminal_p95_mm": record["root"]["terminal_increment"]["xyz_mm"]["norm"]["p95"], "root_terminal_max_mm": record["root"]["terminal_increment"]["xyz_mm"]["norm"]["max"], "pelvis_mae_deg": record["pelvis"]["error_deg"]["mean"], "left_heel_slip_p95_mm_per_frame": record["feet"]["left"]["heel_slip_p95_mm_per_frame"]["p95"], "left_penetration_p95_mm": record["feet"]["left"]["penetration_p95_mm"]["p95"]})
