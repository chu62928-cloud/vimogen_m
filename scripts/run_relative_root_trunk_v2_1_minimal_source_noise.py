#!/usr/bin/env python3
"""Run the single frozen sample94/seed0 root--trunk v2.1 experiment."""
...
PROTOCOL_ROOT = ROOT / "results/phase7/relative_root_trunk_v2_1"
...
def build_config(args, run_root: Path, manifest: Path):
    ...
    config.mode = "eval"
    config.mbench_name = f"relative_root_trunk_v2_1_minimal_delta{args.target_delta_deg:+g}_seed{args.seed}"
    ...
    config.relative_root_forward = {
        ...
        "feasible_relative_mae_deg": float(args.feasible_relative_mae_deg),
        "feasible_relative_p95_deg": float(args.feasible_relative_p95_deg),
...
def run(args) -> dict:
    ...
    try:
        ...
        summaries = list((run_root / "source_noise_artifacts").glob("batch_*/text/guidance_summary.json"))
        if len(summaries) != 1:
        ...
        record["selected_source"] = summary.get("selected_source")
...
def main() -> None:
    ...
    parser.add_argument("--feasible-relative-mae-deg", type=float, default=1.0)
    parser.add_argument("--feasible-relative-p95-deg", type=float, default=2.0)
