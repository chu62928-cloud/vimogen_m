#!/usr/bin/env python3
"""Evaluate one completed v2 generation run without another model call."""
...
from evaluation.absolute_mean_pelvis_v2 import evaluate_single, summarize_rows
from motion_rep.consistency_v2 import reconcile_motion_tensor_v2
from motion_rep.phase1 import MOTION_LAYOUT
from sampling.absolute_mean_pelvis_guidance_v2 import pelvis_angle_curve
...
def evaluate_run(run_root: Path) -> dict:
    ...
    for m0_dir, guided_dir in zip(m0_dirs, guided_dirs):
        ...
        # make its authority comparable to G0/G1 through the same v2 boundary.
        m0_result = reconcile_motion_tensor_v2(
            m0_norm,
            mean=mean,
            std=std,
    ...
    output = {"protocol": "vimogen_absolute_mean_pelvis_v2_full_fk_sagittal", "run_root": str(run_root), "split": record["split"], "seed": seed, "target_mean_deg": target, "summaries": summaries, "per_frame_csv": str(angle_path), "per_action_csv": str(metric_path)}
    (summary_dir / "summary.json").write_text(json.dumps(output, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    lines = ["# Absolute mean pelvis v2 run summary", "", f"- Split: {record['split']}", f"- Seed: {seed}", f"- Absolute target: +{target:g} deg", "", "| Method | Median error (deg) | <=2 deg rate | Curve corr. | Std ratio | J-FK max (m) | dJ max (m) | dR max (deg) | dT max (m) | G1 trigger rate |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for method in ("G0", "G1"):
