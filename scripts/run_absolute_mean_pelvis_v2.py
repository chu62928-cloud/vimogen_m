#!/usr/bin/env python3
"""Run one non-overwriting full-FK sagittal absolute-mean v2 unit."""
...
PROTOCOL_ROOT = ROOT / "results/phase6/absolute_mean_pelvis_v2"
V3_STATE = (
    ROOT
...
def build_config(
    *,
    ...
    config.mode = "eval"
    config.mbench_name = (
        f"absolute_mean_v2_{split}_target{int(target_mean_deg)}_seed{seed}"
    )
...
def run(args) -> dict:
    ...
    if not protocol.is_file() or not input_manifest.is_file():
        raise FileNotFoundError("v2 protocol and split must be frozen before generation")
    run_base = (
        PROTOCOL_ROOT
        / "runs"
        / args.split
